...
def feature_extract(samples, n):
    # 顶部电容与底部电容比值 需改
    top_bottom = samples[:, 9]/samples[:, 4]
    # 相邻极板电容平均值
    ...
    # 1/3高度处电容平均值 需改
    # sfm1 = (samples[:,0]+samples[:,14])/2
    # 上面3个电容平均值
    u3vm = (samples[:, 5]+samples[:, 9]+samples[:, 12])/3
    # 2/3高度处电容平均值 需改
    # sfm2 = (samples[:,5]+samples[:,12])/2
    # 下面3个电容平均值
    d3vm = (samples[:, 0]+samples[:, 4]+samples[:, 14])/3
    # 所有电容平均值
